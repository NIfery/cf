	...
	$(function(){
		...
		$('#delete').click(function(){
			...
			if(chkLen==0){
				...
				$('form[name="frmList"]').prop('action', '<c:url value="/admin/adminDeleteMulti"/>');
...
<!-- 페이징 처리를 위한 form -->
<form action="<c:url value='/admin/adminship'/>" 
                                    ...
                                    <tbody>
                                       ...
                                       <c:set var="idx" value="0"/>
												...
												<tr>
													    <td>
			                                                <label class="au-checkbox">
			                                                    <input type="checkbox" name="selectAdmin[${idx }].adminNo" value="${vo.adminNo }">
			                                                    <span class="au-checkmark"></span>
			                                                </label>
			                                            </td>
														<td>${vo.adminNo}</td>
							...
							<!--  페이지 번호 끝 -->
		                        ...
		                        <div class="divSearch">
								   	<form name="frmSearch" method="post" 
								   		action='<c:url value="/admin/adminship"/>'>
								        <select name="searchCondition">
								            <option value="admin_Id" 
								            	<c:if test="${param.searchCondition == 'adminId' }">            	
								            		selected="selected"
								            	</c:if>
								            >아이디</option>
								            <option value="admin_Name" 
								            	<c:if test="${param.searchCondition == 'adminName' }">            	
								            		selected="selected"
								            	</c:if>
								            >이름</option>
