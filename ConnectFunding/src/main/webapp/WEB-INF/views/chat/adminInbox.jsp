			...
			<div class="container">
				<div class="row">
					...
					<h1 class="title-5 m-b-35">받은 쪽지함(${pagingInfo.totalRecord })</h1>
						<form name="frmSearch" method="post" action='<c:url value="/chat/adminInbox"/>'>
							<div class="row m-3">
								<label>조회기간</label>
								<div class="col-md-2">
									<input type="text" name="startDay" id="startD" class="form-control form-control-sm" value="${searchVOChat.startDay }">
								</div>
								~
								<div class="col-md-2">
									<input type="text" name="endDay" id="endD" class="form-control form-control-sm" value="${searchVOChat.endDay }">
								</div>
								<div class="col-md-2">
											...
											<c:forEach var="vo" items="${list }">
												<tr class="tr-shadow">
													<td><label class="au-checkbox"> 
															<input type="checkbox" name="sMessage[${idx }].messageNo" value="${vo.messageNo }"> 
																<span class="au-checkmark" ></span>
														</label>
													</td>
													<td><span class="block-email">${vo.userNo }</span></td>
													<td>
														<a href="<c:url value='/chat/adminDetail?messageNo=${vo.messageNo }&currentPage=${param.currentPage!=null?param.currentPage:1}&searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }&startDay=${param.startDay}&endDay=${param.endDay }'/>">
													...
													<td>
														<c:if test="${fn:length(vo.messageContent)>25 }">
															${fn:substring(vo.messageContent, 0, 25) }...
														</c:if> 
														<c:if test="${fn:length(vo.messageContent)<=25 }">
															${vo.messageContent}
														</c:if>
