...
CREATE TABLE SECOND_CATEGORY (
	...
	CATEGORY_NAME VARCHAR2(100) NOT NULL, /* 카테고리이름 */
	FIRST_CATEGORY_NO NUMBER /* 대분류번호 */
);
...
COMMENT ON COLUMN SECOND_CATEGORY.FIRST_CATEGORY_NO IS '대분류번호';
...
ALTER TABLE SECOND_CATEGORY
	ADD
		CONSTRAINT FK_FIRST_TO_SECOND_CATEGORY
		FOREIGN KEY (
			FIRST_CATEGORY_NO
		)
...
ALTER TABLE MESSAGE_RECEIVE
	ADD
		CONSTRAINT FK_MESSAGE_SEND_TO_RECEIVE
		FOREIGN KEY (
			MESSAGE_NO
		)
